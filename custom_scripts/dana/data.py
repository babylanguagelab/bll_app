...
naptime_folders = (#'C:/Experimental Data/Daycare Study/ADEX Output/Jamie/naptime data/Daycare Centre Data/',
                   ...
                   'C:/Users/Wayne/Documents/baby-lab/bll/dana/naptime/Home Data/',
                   )
...
def populate_activities(db):
    ...
    rows = list(reader)
    print '%d activity rows found' % (len(rows))
...
def populate_naptime(db, naptime_folder):
    ...
    print '%d naptime files found' % (len(naptime_filenames))
...
def process_src_file(db, src_file, dest_file, stats_file):
    ...
    for i in range(len(rows)):
        accum_start = float(rows[i]['abs_start_time'])
        accum_end = float(rows[i]['abs_end_time'])
