...
in_dirs = ('C:/Users/Wayne/Documents/baby-lab/bll/mel/test_out15/10 mo/',
           'C:/Users/Wayne/Documents/baby-lab/bll/mel/test_out15/14 mo/'
)

out_dir = 'C:/Users/Wayne/Documents/baby-lab/bll/mel/test_out15/'
...
mother_pt1_pitch real,
mother_pt2_pitch real,
mother_pitch_delta real,
mother_pitch_category text,
baby_start_time real,
baby_stop_time real,
baby_duration real,
baby_lag_time real,
baby_max_pitch real,
...
def maternalUttType(db, filenames, ages):
    ...
    writer.writerow(('filename', 'age', 'Rising', 'Falling', 'Neutral', 'Other', 'Rising', 'Falling', 'Neutral', 'Other', 'Rising', 'Falling', 'Neutral', 'Other',))
    
    pitches = ('Rising', 'Falling', 'Neutral', '',)
...
def infantResponseSummary(db, filenames, ages):
    ...
    writer = csv.writer(file_out)
    writer.writerow(('', '', 'Declaratives', '', '', '', 'YN', '', '', '', 'WH', '', '', '',))
    writer.writerow(('filename', 'age', 'Mother Rise', 'Mother Fall', 'Mother Neutral', 'Mother Other', 'Mother Rise', 'Mother Fall', 'Mother Neutral', 'Mother Other','Mother Rise', 'Mother Fall', 'Mother Neutral', 'Mother Other',))
        ...
        for j in range(len(sentence_types)):
            ...
            for cur_pitch in pitches:
                db_row = db.select(
                    'reorg2',
                    ('count(id)',),
                    where_cond="filename=? AND speaker=? AND sentence_type=? AND question_type=? AND mother_pitch_category=? AND (baby_start_time != '' or baby_stop_time != '')",
                    params=(cur_filename, 'M', cur_stype, cur_qtype, cur_pitch,)
                )
                csv_row.append(db_row[0][0])
...
def infantResponseDeclarative(db, filenames, ages):
    ...
    writer = csv.writer(file_out)
    writer.writerow(('', '', 'Mother Rise', '', '', '', 'Mother Fall', '', '', '', 'Mother Neutral', '', '', '', 'Mother Other', '', '', ''))
    writer.writerow(('filename', 'age',
                     'Infant Rise', 'Infant Fall', 'Infant Neutral', 'Infant Other',
                     'Infant Rise', 'Infant Fall', 'Infant Neutral', 'Infant Other',
                     'Infant Rise', 'Infant Fall', 'Infant Neutral', 'Infant Other',
                     'Infant Rise', 'Infant Fall', 'Infant Neutral', 'Infant Other',
                 ))
