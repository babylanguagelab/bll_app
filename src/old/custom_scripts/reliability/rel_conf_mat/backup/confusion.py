...
def combine_dot_split_utters(utters):
    combined = []
...
def divide_segs(segs):
    ...
    for i in range(len(segs)):
        ...
        #<hack>
        segs[i].utters = combine_dot_split_utters(segs[i].utters)
        #</hack>

        for j in range(len(segs[i].utters)):
            sm.drive(segs[i].utters[j])
        sm.finish()
        
        if sm.state == StateMachine.STATES.ERROR:
            print 'The state machine has reached the error state. You should look into that.'
        
        single.extend(sm.single)
        numbered_multi.extend(sm.numbered_multi)
        unnumbered_multi.extend(sm.unnumbered_multi)
