#This is yet another desparate attempt to write this app :P
...
class MainWindow:
    ...
    def __init__(self, master):
       ...
       # self.submenu.add_command(label = 'Set CDS sample', command = lambda self = self, kind = 'cds': self.set_sample(kind))
        ...
        self.blocks_list = tk.Listbox(self.frame, width = 20, height = 15, exportselection = False, relief = 'flat')
        ...
        self.segments_list = tk.Listbox(self.frame, width = 20, height = 15, exportselection = False, relief = tk.FLAT)
...
#        labelled = len([x for x in self.labelled_data[self.current_subject].keys() if not x.startswith('_')])
 #       total = len(names_sorted)
  #      self.bar.set('{}% of segments are labelled for this recording'.format(100 * labelled / total))
